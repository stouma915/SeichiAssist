...
/**
 * Created by karayuu on 2019/11/16
 */
object SeichiLevelUpMessages {
  private val config: Config = SeichiAssist.seichiAssistConfig

  private type Level = Int
  private val manaIncreaseMessage = "整地の際に獲得できるマナが増えました。"
  private val pocketCapacityIncreaseMessage = "四次元ポケットの容量が増加しました！"

  val messageMap: Map[Level, String] = Map(
    ...
    5 -> s"Dynmapでどこに誰がいるのか、何があるのかなどを見ることができます。\nURLはこちら→${config.getUrl("map")}",
    ...
    9 -> s"整地鯖をもっと良くするために、是非JPMCSに投票をお願いします。\n1日1回投票出来ます。投票すると様々な特典を受け取れます！\nURLはこちら→${config.getUrl("jms")}",
    ...
    18 -> s"$manaIncreaseMessage\n経験値量は10レベルごとに増加し、スキルがより長く使えるようになります！",
    20 -> "新しいアクティブスキル[トリアルブレイク]を習得できます！\n習得するには木の棒メニューのスキルブックをクリック！",
    25 -> "どこでもエンダーチェスト機能を解除しました！\n木の棒メニューから呼び出せます。",
    28 -> manaIncreaseMessage,
    30 -> "新しいアクティブスキル[エクスプロージョン]を習得できます！\n習得するには木の棒メニューのスキルブックをクリック！",
    38 -> manaIncreaseMessage,
    40 -> "新しいアクティブスキルを習得できます！\n習得するには木の棒メニューのスキルブックをクリック！\nツリーが分かれていますので、お好きなスキルをドウゾ。",
    46 -> pocketCapacityIncreaseMessage,
    48 -> manaIncreaseMessage,
    56 -> pocketCapacityIncreaseMessage,
    58 -> manaIncreaseMessage,
    66 -> pocketCapacityIncreaseMessage,
    68 -> manaIncreaseMessage,
    78 -> manaIncreaseMessage,
    88 -> manaIncreaseMessage,
    98 -> manaIncreaseMessage
  )

  def getMessage(level: Level): String = messageMap.getOrElse(level, "")
}
